...
{
    printf("DebugPrint");
    printf("cur_id:%lu, isSw:%d", cur_id, isSw);
    printf("id_segment[0], min_id:%lu, max_id:%lu", id_segment[0].min_id, id_segment[0].max_id);
    printf("id_segment[1], min_id:%lu, max_id:%lu", id_segment[1].min_id, id_segment[1].max_id);
}
...
int LocalShmMemKV::Init(key_t key, int flags, uint64_t num)
{
	...
	if(shmid == -1) {
		...
		if(shmid == -1) {
			printf("shmid == -1, %s", strerror(errno));
			return -1;
		}
		is_first = true;
	} else {
	...
	if(real_size != (sizeof(KVHead) + sizeof(KVItem) * num)) {
		printf("real_size not match, real_size:%d, size:%lu", real_size, (sizeof(KVHead) + sizeof(KVItem) * num));
		return -1;
	}
	...
	if(is_first) {
		printf("First use that share memory[%d], clear all", real_size);
		...
		printf("Use existed share memory, shm size; %d", real_size);
	...
	if(status) {
		printf("init mutexattr failed.%d", status);
		return -1;
	}

	status = pthread_mutexattr_setpshared(&mutex_attr, PTHREAD_PROCESS_SHARED);
	if(status) {
		printf("set mutex attr to shared failed. %d", status);
		return -1;
	}
...
int LocalShmMemKV::GetNextSeq(const int cur_pos, uint64_t & seq)
{
	...
	printf("wsq. m_pKVCache->items[cur_pos].key:%s",m_pKVCache->items[cur_pos].key);
	//左开右闭区间（min_id,max_id]
	if(tmp_seq.cur_id < tmp_seq.id_segment[tmp_seq.isSw].max_id )
	{
		tmp_seq.cur_id += 1;
		seq = tmp_seq.cur_id;
		printf("hit this segment. isSw:%d, cur_id:%lu, max_id:%lu", tmp_seq.isSw, tmp_seq.cur_id, tmp_seq.id_segment[tmp_seq.isSw].max_id );
	}
	else if( tmp_seq.cur_id == tmp_seq.id_segment[tmp_seq.isSw].max_id &&  tmp_seq.cur_id <= tmp_seq.id_segment[1-tmp_seq.isSw].min_id)
	{
		...
		printf("hit another segment. isSw:%d, cur_id:%lu, max_id:%lu", tmp_seq.isSw, tmp_seq.cur_id, tmp_seq.id_segment[tmp_seq.isSw].max_id);
	}
	else
	{
		//需要从kv or zk中取号段;
		...
		{
			//查询kv失败
			printf( "ERR: call kv failed! ret:%d, key:%s", ret, m_pKVCache->items[cur_pos].key);
			return ret;
		}
		//刷新缓存号段
		...
		seq = tmp_seq.cur_id;
		printf("hit kv. isSw:%d, cur_id:%lu, max_id:%lu", tmp_seq.isSw, tmp_seq.cur_id, tmp_seq.id_segment[tmp_seq.isSw].max_id);
	}
	return 0;
}
...
int LocalShmMemKV::GetAndInrcSeq(std::string &key, uint64_t & seq)
{
	...
	if(m_pKVCache->items[m_pKVCache->head.cur_list_end].used) {
		//no bucket
		UnlockHead();
		printf("used: %d, no bucket", m_pKVCache->items[m_pKVCache->head.cur_list_end].used);
		return -1;
	}
	...
	if( 0 != ret )
	{
		//查询kv失败
		UnlockItem(list_end);
		printf( "ERR: call kv failed! ret:%d, key:%s", ret, key.c_str());
		return ret;
	}
	...
	printf("hit kv and set mem cache. isSw:%d, cur_id:%lu, max_id:%lu", tmp_seq.isSw, tmp_seq.cur_id, tmp_seq.id_segment[tmp_seq.isSw].max_id);
	UnlockItem(list_end);
...
{
	auto & tmp_seq = m_pKVCache->items[cur_pos].seq_segment;
	printf("FlushSegment segment key:%s", m_pKVCache->items[cur_pos].key);
	tmp_seq.DebugPrint();
	//左开右闭区间（min_id,max_id]
	if(tmp_seq.cur_id >= tmp_seq.id_segment[tmp_seq.isSw].GetMiddleId() && tmp_seq.cur_id >= tmp_seq.id_segment[1-tmp_seq.isSw].max_id )
	{
		...
		{
			//查询kv or zk 失败
			printf( "ERR: flsuh kv_key failed! ret:%d, key:%s", ret, m_pKVCache->items[cur_pos].key);
			return ret;
		}
		//刷新缓存号段
		tmp_seq.id_segment[1-tmp_seq.isSw].max_id = max_seq;
		tmp_seq.id_segment[1-tmp_seq.isSw].min_id = max_seq - step;
		printf( "ERR: flsuh kv_key suc! key:%s, min_id:%lu, max_id:%lu", m_pKVCache->items[cur_pos].key, tmp_seq.id_segment[1-tmp_seq.isSw].min_id, max_seq);
	}
	return 0;
}
...
int LocalShmMemKV::GetItem(std::string &key, uint64_t &count, int &key_time, int pos)
{
	...
	if(pos < 0 || pos >= STAT_NUM) {
		printf("panic, pos: %d not at the right range[0, %d)", pos, STAT_NUM);
		return -1;
	}
	...
	if(vec.size() != 2) {
		...
		printf("bad name at share memory: %s, skip", key.c_str());
		return  -2;
	}
